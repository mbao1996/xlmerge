...
def merge_sheet(wb, sheet, cfg):
    ...
    for i in range(row_start, ws.max_row+1):
        op = ws.cell(i, col_opr).value          # get opr
        op = ''.join(op.split())
    ...
    for k in range(len(opr)):   # every files to be merged
        ...
        sheets_rd = wb_rd.sheetnames
        if sheet not in sheets_rd:
            print('sheet:', sheet, ' is not exist in file:', fn_rd)
            return(wb)
        ws_rd = wb_rd[sheet]
        rd = None
        rd_sn = 0
...
for i in range(len(sheets)):
    print(sheets[i])
    wb = merge_sheet(wb, sheets[i], cfg)

time_flag = time.strftime("%Y%m%d_%H%M%S", time.localtime())
fn_div = fn.split('.')
fn_wr = fn_div[0] + '_' + time_flag + '.xlsx'
save_fn(wb, fn_wr)
